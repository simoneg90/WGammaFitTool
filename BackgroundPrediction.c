...
void BackgroundPrediction(std::string suffix,std::string filename, int rebin_factor,int model_number = 0,int imass=750, std::string Model="narrow", std::string dirAppend="", bool plotBands = false)
{
    std::cout<<"Welcome to the Background program"<<std::endl; 
    rebin = rebin_factor;
//    std::string fname = std::string("../fitFilesMETPT34/") + suffix + std::string("/histos_bkg.root");
//    std::string fname = dirname + suffix + std::string("/histos_bkg.root");
    std::string fname = std::string(filename);
    std::cout<<"Fname: "<<fname.c_str()<<std::endl;
 
    stringstream iimass ;
    iimass << imass;
    std::string dirName = "info_"+iimass.str()+"_"+suffix+"_"+Model+dirAppend;
    ...
    std::cout<<"FileName after opening: "<<fname.c_str()<<std::endl;
    
    std::string histoName="dataHisto"; //added to interplay with my analysis flow
...
//    TH1D *h_mX_CR_tau;//=(TH1F*)file->Get("distribs_1_10_1")->Clone("CR_tau"); //=== apparently used as the main histo for the fit ===


    TH1D *h_mX_CRb=(TH1D*)file->Get(histoName.c_str());
    TH1D *h_mX_CRab=(TH1D*)file->Get(histoName.c_str());
    TH1D *h_mX_CRabt=(TH1D*)file->Get(histoName.c_str());
    TH1D *h_mX_CRabat=(TH1D*)file->Get(histoName.c_str());
    TH1D *h_mX_CR_tau=(TH1D*)file->Get(histoName.c_str());//=(TH1F*)file->Get("distribs_1_10_1")->Clone("CR_tau"); //=== apparently used as the main histo for the fit ===
    //if(suffix=="antibtag")h_mX_CR_tau =(TH1D*) h_mX_CRab->Clone();
    //else if(suffix=="btag")h_mX_CR_tau = (TH1D*)h_mX_CRb->Clone();
    //else if(suffix=="tau")h_mX_CR_tau = (TH1D*)h_mX_CRabt->Clone();
    //else if(suffix=="anti")h_mX_CR_tau = (TH1D*)h_mX_CRabat->Clone();

    std::cout<<"After declaring histos"<<std::endl;
...
//    TH1D *h_mX_abat=(TH1D*)file->Get("distribs_5_10_0");
    ...
    std::cout<<"After declaring signal region histos"<<std::endl;

    h_mX_SR->SetName("The_SR");

    double maxdata = h_mX_SR->GetMaximum();
    double nEventsSR = h_mX_SR->Integral(h_mX_SR->FindBin(600),h_mX_SR->FindBin(4000));
    std::cout<<"maxdata: "<<maxdata<<std::endl;
    std::cout<<"nEventsSR: "<<nEventsSR<<std::endl;
...
*/    
    ...
    std::cout<<"Before declaring and cloning histos"<<std::endl;
    ...
    std::cout<<"After declaring and cloning histos"<<std::endl;
    
    /*TFile *f_sig = new TFile((dirName+"/w_signal_"+iimass.str()+".root").c_str());
    RooWorkspace* xf_sig = (RooWorkspace*)f_sig->Get("Vg");
    RooAbsPdf *xf_sig_pdf = (RooAbsPdf *)xf_sig->pdf((std::string("signal_fixed_")+suffix).c_str());
    
    RooWorkspace w_sig("w");
    w_sig.import(*xf_sig_pdf,RooFit::RenameVariable((std::string("signal_fixed_")+suffix).c_str(),(std::string("signal_fixed_")+suffix+std::string("low")).c_str()),RooFit::RenameAllVariablesExcept("low","x"));
    xf_sig_pdf = w_sig.pdf((std::string("signal_fixed_")+suffix+std::string("low")).c_str());
...
//    RooRealVar x("x", "m_{X} (GeV)", SR_lo, SR_hi);
    ...
    RooRealVar nBackground((std::string("bg_")+suffix+std::string("_norm")).c_str(),"nbkg",0.5*nEventsSR);//h_mX_SR->GetSumOfWeights());
    std::cout<<"Weights: "<<h_mX_SR->GetSumOfWeights()<<" Integral "<<h_mX_SR->Integral()<<std::endl;
    RooRealVar nBackground2((std::string("alt_bg_")+suffix+std::string("_norm")).c_str(),"nbkg",h_mX_SR->GetSumOfWeights());
    std::string cat = suffix;
    //suffix=""; //Antibtag=tag to constrain b-tag to the anti-btag shape
    
    
    /* RooRealVar bg_p0((std::string("bg_p0_")+suffix).c_str(), "bg_p0", 4.2, 0, 200.);
     RooRealVar bg_p1((std::string("bg_p1_")+suffix).c_str(), "bg_p1", 4.5, 0, 300.);
     RooRealVar bg_p2((std::string("bg_p2_")+suffix).c_str(), "bg_p2", 0.000047, 0, 10.1);
     RooGenericPdf bg_pure = RooGenericPdf((std::string("bg_pure_")+cat).c_str(),"(pow(1-@0/13000,@1)/pow(@0/13000,@2+@3*log(@0/13000)))",RooArgList(x,bg_p0,bg_p1,bg_p2));
   */
    RooRealVar bg_p0((std::string("bg_p0_")+suffix).c_str(), "bg_p0", 0., -1000, 200.);
    RooRealVar bg_p1((std::string("bg_p1_")+suffix).c_str(), "bg_p1", -13, -1000, 1000.);
    RooRealVar bg_p2((std::string("bg_p2_")+suffix).c_str(), "bg_p2", -1.4, -1000, 1000.);
    bg_p0.setConstant(kTRUE);
    //RooGenericPdf bg_pure = RooGenericPdf((std::string("bg_pure_")+cat).c_str(),"(pow(@0/13000,@1+@2*log(@0/13000)))",RooArgList(x,bg_p1,bg_p2));
    RooGenericPdf bg = RooGenericPdf((std::string("bg_")+cat).c_str(),"(pow(@0/13000,@1+@2*log(@0/13000)))",RooArgList(x,bg_p1,bg_p2));
    ...
    if ((imass > 2450 && cat == "antibtag") || (imass > 1640 && cat == "btag")) {
        if (cat == "antibtag") {
    ...
    RooRealVar bias_term((std::string("bias_term_")+cat).c_str(), "bias_term", 0., -bias_term_s, bias_term_s);
    //bias_term.setConstant(kTRUE);
    RooAddPdf bg((std::string("bg_")+cat).c_str(), "bg_all", RooArgList(*xf_sig_pdf, bg_pure), bias_term);
    */
...
//    RooDataSet  pred("pred", "Prediction from SB", inNT, RooArgSet(x,isSR),"isSB_at > 0.5");
    ...
    //std::cout<< "bg_p0_"<< suffix << "   param   "<<bg_p0.getVal() <<  " "<<bg_p0.getError()<<std::endl;
    std::cout<< "bg_p1_"<< suffix << "   param   "<<bg_p1.getVal() <<  " "<<100*bg_p1.getError()<<std::endl;
    std::cout<< "bg_p2_"<< suffix << "   param   "<<bg_p2.getVal() <<  " "<<100*bg_p2.getError()<<std::endl;
    //std::cout<< "bias_term_"<< cat << "   param   0 "<<bias_term_s<<std::endl;
...
//    frameP->GetXaxis()->SetRangeUser(600,1000);
    ...
    if(bias){
        ...
        for(int i=model_number; i<=model_number; i++){
            ...
            w_alt->import(*alt_bg, RooFit::RenameVariable(alt_bg->GetName(),("alt_bg_"+cat).c_str()));
            w_alt->Print("V");
            std::cerr<<w_alt->var("x")<<std::endl;
            RooRealVar * range_ = w_alt->var("x");
            range_->setRange(SR_lo,SR_hi);
            char* asd = (char *)(("alt_bg_"+cat).c_str());
        ...
        p_1->SetLogy();
        c_rooFit->Update();
        c_rooFit->SaveAs((dirName+"/"+name_output+cat+"_multipdf.pdf").c_str());
    ...
    std::cout<<"nEventsSR: "<<nEventsSR<<" getSumW: "<<h_mX_SR_fakeData->GetSumOfWeights()<<std::endl;;
